...
namespace Iridium
{
    bool Tfit2Context::Load(BufferedStream& input)
    {
        if (input.Read(&InitTables, sizeof(InitTables)) != sizeof(InitTables))
            return false;
        ...
        for (usize i = 0; i < 17; ++i)
        {
            ...
            if (input.Read(&round.Lookup, sizeof(round.Lookup)) != sizeof(round.Lookup))
                return false;

            for (usize j = 0; j < 16; ++j)
            {
                auto& block = round.Blocks[j];

                if (input.Read(&block.Masks, sizeof(block.Masks)) != sizeof(block.Masks))
                    return false;

                if (input.Read(&block.Xor, sizeof(block.Xor)) != sizeof(block.Xor))
                    return false;
        ...
        if (input.Read(&EndMasks, sizeof(EndMasks)) != sizeof(EndMasks))
            return false;

        if (input.Read(&EndTables, sizeof(EndTables)) != sizeof(EndTables))
            return false;
    ...
    {
#define X(NAME) f.Add(hashes.NAME, &NAME, sizeof(NAME))
        ...
        for (usize i = 0; i < 17; ++i)
        {
            ...
            for (usize j = 0; j < 16; ++j)
            {
                X(Rounds[i].Blocks[j].Masks);
                X(Rounds[i].Blocks[j].Xor);
            }
        }
...
#undef X
    ...
    bool Tfit2Context::Save(BufferedStream& output)
    {
        if (output.Write(&InitTables, sizeof(InitTables)) != sizeof(InitTables))
            return false;
        ...
        for (usize i = 0; i < 17; ++i)
        {
            ...
            if (output.Write(&round.Lookup, sizeof(round.Lookup)) != sizeof(round.Lookup))
                return false;
            ...
            for (usize j = 0; j < 16; ++j)
            {
                ...
                if (output.Write(&block.Masks, sizeof(block.Masks)) != sizeof(block.Masks))
                    return false;

                if (output.Write(&block.Xor, sizeof(block.Xor)) != sizeof(block.Xor))
                    return false;
        ...
        if (output.Write(&EndMasks, sizeof(EndMasks)) != sizeof(EndMasks))
            return false;

        if (output.Write(&EndTables, sizeof(EndTables)) != sizeof(EndTables))
            return false;
    ...
    static IR_FORCEINLINE void TFIT2_DecryptSplatBytes(u8 bytes[8], u64 output[8])
    {
        for (usize i = 0; i < 8; ++i)
    ...
    static void TFIT2_DecryptRoundB(const Tfit2Context::Round& round, u8 data[16], const u64 key[2])
    {
        u64 v0[8], v1[8];
    ...
    static void TFIT2_DecryptRoundC(const Tfit2Context::Round& round, u8 data[16], const u64 key[2])
    {
        u64 v0[8], v1[8];
    ...
    static IR_FORCEINLINE u8 TFIT2_DecryptSquashBytes(const u64 input[8], const u64 lookup[8])
    {
    ...
    static void TFIT2_DecryptBlock(const Tfit2Context& ctx, const u64 key[17][2], const u8 input[16], u8 output[16])
    {
        ...
        TFIT2_DecryptRoundB(ctx.Rounds[0], temp, key[0]);
        TFIT2_DecryptRoundB(ctx.Rounds[1], temp, key[1]);

        for (usize i = 2; i < 16; ++i)
            TFIT2_DecryptRoundC(ctx.Rounds[i], temp, key[i]);

        TFIT2_DecryptRoundB(ctx.Rounds[16], temp, key[16]);
    ...
    Tfit2CbcCipher::Tfit2CbcCipher(const u64 keys[17][2], const u8 iv[16], const Tfit2Context* ctx)
        : keys_(keys)
        , ctx_(ctx)
    {
